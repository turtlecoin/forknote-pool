...
//TODO find a way for no sudo..
var b_https = false;
if((config.api.path_privkey != "") && (config.api.path_fullchain != ""))
{
  const options = {
    key: fs.readFileSync(config.api.path_privkey),
    cert: fs.readFileSync(config.api.path_fullchain)
  };
  b_https = true;
}
...
if(b_https){
  serverhttps.listen(config.api.https_port, function () {
