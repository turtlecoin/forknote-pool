...
var addressBanningEnabled = config.poolServer.addressBanning && config.poolServer.addressBanning.enabled;
...
setInterval(function(){
    ...
    if (addressBanningEnabled) {
        for (address in bannedAddress) {
          var banTime = bannedAddress[address];
          if (now - banTime > config.poolServer.banning.time * 1000) {
            delete bannedAddress[address];
            delete perAddressStats[address];
...
process.on('message', function(message) {
    switch (message.type) {
        case 'banIP':
            ...
            break;
        case 'banAddress':
            bannedAddress[message.login] = Date.now();
            break;
    }
});
...
function IsBannedAddress(address){
    if (!addressBanningEnbled || !bannedAddress[address]) return false;

    var bannedTime = bannedAddress[address];
    var bannedTimeAgo = Date.now() - bannedTime;
    var timeLeft = config.poolServer.addressBanning.time * 1000 - bannedTimeAgo;
    if (timeLeft > 0){
		    return true;
    }
	  else {
...
Miner.prototype = {
    ...
    checkAddressBan: function(validShare){
        if (!addressBanningEnabled) return;

        // Init global per-address shares stats
        if (!perAddressStats[this.login]){
            perAddressStats[this.login] = { validShares: 0, invalidShares: 0 };
        }

        var stats = perAddressStats[this.login];
        validShare ? stats.validShares++ : stats.invalidShares++;
        if (stats.validShares + stats.invalidShares >= config.poolServer.addressBanning.checkThreshold){
            if (stats.invalidShares / (stats.invalidShares + stats.validShares) >= config.poolServer.addressBanning.invalidPercent / 100) {
                log('warn', logSystem, 'Banned %s@%s', [this.login, this.ip]);
                bannedAddress[this.login] = Date.now();
                delete connectedMiners[this.id];
                process.send({type: 'banAddress', login: this.login});
            }
            else{
                stats.invalidShares = 0;
                stats.validShares = 0;
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    if (params.login && IsBannedAddress(params.login)){
    ...
    switch(method){
	    ...
	    if (!noncePattern.test(params.nonce)) {
                ...
                if (!perAddressStats[miner.login]) {
                    perAddressStats[miner.login] = { validShares: 0, invalidShares: 0 };
                }
                perAddressStats[miner.login].invalidShares += Math.floor((config.poolServer.addressBanning.checkThreshold / 4) * (config.poolServer.addressBanning.invalidPercent / 100) -1);

                miner.checkBan(false);
                miner.checkAddressBan(false);
            ...
            miner.checkAddressBan(shareAccepted);
